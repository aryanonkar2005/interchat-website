...
export function Logo({ className, size = 24, withText = true }: LogoProps) {
  return (
    <div onClick={handleClick} className={cn("flex items-center sm:gap-4 gap-2.5 cursor-pointer", className)}>
      ...
      {withText && (
        <span className="font-medium text-xl mt-px text-primary dark:text-white select-none">InterChat</span>
      )}
    </div>
  );
}
